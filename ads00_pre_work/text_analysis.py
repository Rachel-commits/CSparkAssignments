...
def extract_numbers(text):
    ...
    for word in new_text:
        ...
        stripped_text += stripped_word
        stripped_text += ' '
    p = re.compile(r'\d+')
    new_list = p.findall(stripped_text)
...
def latin_ish_words(text):
    ...
    new_text = text.split()
    for word in new_text:
        latin_text = ['tion', 'ex', 'ph', 'ost', 'ist', 'ast']
        if any(x in word.lower() for x in latin_text):
            new_list.append(word.translate(
