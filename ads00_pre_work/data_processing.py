...
def normalize_prices(prices):
    ...
    :rtype: list
    """
    initial_hours = len(prices[0])
    if prices[0][0] == 0:
        raise ValueError
    norm_factor = 100/prices[0][0]
    norm_prices = []
    for day in prices:
        if len(day) != initial_hours:
            raise ValueError
        day = [norm_factor*hour for hour in day]
        norm_prices.append(day)
...
def flip_prices(prices):
    ...
    :rtype: list
    """
    # check all memebers of the list are the same length
    initial_hours = len(prices[0])
    for day in prices:
        if len(day) != initial_hours:
            raise ValueError

    flippedlist = list(map(list, zip(*prices)))
