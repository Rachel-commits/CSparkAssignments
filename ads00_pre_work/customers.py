...
def process_clients(segment):
    ...
    for cust in segment:
        ...
        if cust.get('address') is not None:
            # print(cust.get('first-name'),cust.get('middle-name',""),cust.get('address'))
            name = (cust.get('title', "")+" " + cust.get('first-name', "") +
                    " " + cust.get('middle-name', "") + " " + cust.get('last-name', ""))
            details = (" ".join(name.split()), cust.get("address"))
            customers.append(details)

    return customers

# clients = process_clients(CLIENTS_EXAMPLE)
# print(clients)
